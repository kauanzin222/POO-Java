...
import fatec.poo.models.PedidoCompra;
...
public class App {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < qntdPes; i++) {
            ...
            System.out.print("Informe quantos pedidos de compra a pessoa fez: ");
            int qntdPed = sc.nextInt();
            sc.nextLine();

            for (int j = 0; j < qntdPed; j++) {
                System.out.println("\n---\tPedido #" + (j+1));
                
                PedidoCompra ped = new PedidoCompra(j + 1);
                System.out.print("Informe a data do pedido: ");
                ped.setDataPedido(LocalDate.parse(sc.next(), fmt1));
                System.out.print("Informe o valor do pedido R$");
                ped.setValor(sc.nextDouble());
                
                list.get(i).addCompras(ped);
            }
        }
